...
/* START METADATA HACK */
										'category_title' => vmGet($d, 'category_title'),
										'category_metakey' => vmGet($d, 'category_metakey'),
										'category_metadesc' => vmGet($d, 'category_metadesc'),
										'category_abstract' => vmGet($d, 'category_abstract'),
                        						'category_canonical' => vmGet($d, 'category_canonical'),
...
// METADATA HACK START - Update DB 
		...
		$fields_metaglb = array ( 'banned' => vmGet($d,'banned'),
	 						'remove_less_than' => vmGet($d,'remove_less_than'),
                        			...
                        			'append_subcategory_title' => vmGet($d, 'append_subcategory_title'),
                        			'append_subcategory_description' => vmGet($d, 'append_subcategory_description'),
                        			'prepend_cat_name_phrase' => vmGet($d, 'prepend_cat_name_phrase'),
                        			'prepend_par_cat_name_phrase' => vmGet($d, 'prepend_par_cat_name_phrase'),
 		...
 		$dbmc->buildQuery( 'UPDATE', '#__{vm}_category_metakeys', $fields_metaglb,  'WHERE mid!=2' );
 		$dbmc->query();
// METADATA HACK END.. MORE BELOW
...
// METADATA HACK END. END.
	...
	*/
	...
	function get_filter_entries( $category_id, $count )
	{
	    global $sess;
	    
	    $html .= '<a title="Показать все ('.$count.')" class="fentries_i1" href="'. $sess->url("index.php?page=shop.browse&amp;category_id=".$category_id) . '">';
	    $html .= 'Показать все (' . $count . ' )</a>';
	    return $html;
	}
	
	function get_category_tree_by_node( $category_id, & $html, $links_css_class, $list_css_class, $highlighted_style, $node = NULL, $level = 0)
	{
	    global $sess;
		...
		if (($el['parent'] >= 0) && ($level > 0))
		{
		    ...
		    if( $category_id == $el['node']['category_id'] ) 
		    {
		    ...
		    $html .= '<li>';
		    if($level == 1)
			$class = $links_css_class;
		    else
			$class = $list_css_class;
		    $count = $el['node']['category_products'];
		    
		    //if (!empty($el['sub']))
		    //	$count += $this->products_in_all_subcategories($el['sub']);
	
			    
			$html .= '<a title="'.htmlspecialchars($el['node']['category_name']).'" style="'.$style .'" class="'. $class . '" ';

			$html .= $append . ' href="'. $sess->url("index.php?page=shop.browse&amp;category_id=".$el['node']['category_id']) . '">';
			//$html .= htmlspecialchars($el['node']['category_name']) . ' (' . $count . ')</a>';
			if ($level == 1)
			    $html .= '<span class="mitem_common mitem' . $el['node']['category_id'] . '"></span>';
			
			$html .= htmlspecialchars($el['node']['category_name']) . '</a>';
		...
		{
		    if ($level == 0) {
			$html .= '<ul id="'.$links_css_class.'" class="accordion">';
			//$html .= '<ul id="'.$links_css_class.'">';

		    }else
			$html .= '<ul>';

		    $this->get_category_tree_by_node($category_id, $html, $links_css_class, $list_css_class, $highlighted_style, $el['sub'], $level+1);
				
		    if ($level == 1) {

			//$html .= '<li class="filter_entries">';
			//$html .= $this->get_filter_entries($el['node']['category_id'], $count);
			//$html .= '</li>';
		    }
		    $html .= '</ul>';
				...
				$highlighted_style="font-style:italic;" ) 
	{
	    ...
	    $toplevel = $psF->get_categories_tree();
	    
	    //$filter_entities = $psF->get_filter_entities_all();
	    //print_r($filter_entities);
	    
	    $html = '<nav>';
		$this->get_category_tree_by_node( $category_id, $html, $links_css_class, $list_css_class, $highlighted_style, $toplevel);
	    $html .= '</nav>';
	    return $html;
	 ...
	 */
	function get_category_tree_old( $category_id=0,
				$links_css_class="mainlevel",
				$list_css_class="mm123",
				$highlighted_style="font-style:italic;" ) {
		global $sess;
		...
		for($n = 0 ; $n < $nrows ; $n++) {
			...
			if( $allowed ) {
				...
				if ($n == 0) {
				    $html .= '<ul id="'. $css_class .'">';
				} else if ($depth_list[$n] > $depth_list[$n - 1]) {
				    $html .= '<ul>';
				} else if ($depth_list[$n] < $depth_list[$n - 1]) {
				    $html .= '</li>';
				    $html .= str_repeat('</ul></li>', $depth_list[$n - 1] - $depth_list[$n]);
				} else {
				    $html .= '</li>';
				}
				...
				$html .= '
          <li><a title="'.$catname.'" style="display:block;'.$style.'" class="'. $css_class .'" href="'. $sess->url(URL."index.php?page=shop.browse&amp;category_id=".$category_tmp[$row_list[$n]]["category_child_id"]) .'" '.$append.'>'
		...
		$html .= str_repeat('</li></ul>', end($depth_list));
		$html .= '</li></ul>';
		return $html;
	}
	...
	*/
	...
	if( PSHOP_SHOW_PRODUCTS_IN_CATEGORY == '1' ) {
	    if (!isset($_SESSION['categories_info'][$category_id]['products_count']))
	    {
		$num = ps_product_category::prod_in_cat_recursive( $category_id );
		$_SESSION['categories_info'][$category_id]['products_count'] = $num;
	    } else {
		$num = $_SESSION['categories_info'][$category_id]['products_count'];
	    }
	    return " ($num) ";
	}
	else
