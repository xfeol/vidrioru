...
class vm_ps_product extends vmAbstractObject {
	...
	function add( &$d ) {
		...
		$fields = array ( 'vendor_id' => $vendor_id,
						...
						'product_price_alternative' => vmGet($d, 'product_price_alternative'),
/* START METADATA HACK */
                        		'product_title' => vmGet($d, 'product_title'),
                        		'product_metadesc' => vmGet($d, 'product_metadesc'),
                        		'product_metakey' => vmGet($d, 'product_metakey'),
                        		'product_abstract' => vmGet($d, 'product_abstract'),
                        		'product_canonical' => vmGet($d, 'product_canonical'),
/* END METADATA HACK.. MORE BELOW */
...
// METADATA HACK START - Update DB 
		$dbmp = new ps_DB;
		$fields_metaglb = array ( 'banned' => vmGet($d,'banned'),
	 						'remove_less_than' => vmGet($d,'remove_less_than'),
                        			'number_of_keywords' => vmGet($d, 'number_of_keywords'),
                        			'append_category' => vmGet($d, 'append_category'),
                        			'general_append_end' => vmGet($d, 'general_append_end'),
                        			'general_append_end_title' => vmGet($d, 'general_append_end_title'),
                        			'general_append_keys' => vmGet($d, 'general_append_keys'),
                        			'general_append_keys_phrase' => vmGet($d, 'general_append_keys_phrase'),
                        			'append_prod_name_phrase' => vmGet($d, 'append_prod_name_phrase'),
                        			'append_par_cat_name_phrase' => vmGet($d, 'append_par_cat_name_phrase'),
 							'turn_on_edit' => vmGet($d, 'turn_on_edit'),
 							'add_abstract' => vmGet($d, 'add_abstract'),
 							'generator' => vmGet($d, 'generator'),
 							'subject' => vmGet($d, 'subject'),
 							'classification' => vmGet($d, 'classification'),
 							'author' => vmGet($d, 'author'),
 							'organization' => vmGet($d, 'organization'),
 							'copyright' => vmGet($d, 'copyright'),
 							'country' => vmGet($d, 'country'),
 							'content_language' => vmGet($d, 'content_language'),
 							'language' => vmGet($d, 'language'),
 							'designer' => vmGet($d, 'designer'),
 							'comments' => vmGet($d, 'comments'),
 							'no_email_collection' => vmGet($d, 'no_email_collection') );
 		$dbmp->buildQuery( 'UPDATE', '#__{vm}_product_metakeys', $fields_metaglb,  'WHERE mid!=2' );
 		$dbmp->query();
// METADATA HACK END.. MORE BLEOW
...
// METADATA HACK END. END.
	...
	function get_fieldALL($product_id_set) {
	    $db = new ps_DB;
	    
	    $q = 'SELECT product_id, `#__{vm}_product`.* FROM `#__{vm}_product` WHERE `product_id` IN ('.$product_id_set.')';
	    $db->query($q);
	    while($db->next_record()) {
		$GLOBALS['product_info'][$db->f('product_id')] = get_object_vars($db->getCurrentRow());
	    }
	}
	...
	{
	    if (empty( $_SESSION['products_info'][$product_id]['info']['flypage']))
	    {
		$_SESSION['products_info'][$product_id]['info']['flypage'] = "flypage.tpl";
	    }
	    return $_SESSION['products_info'][$product_id]['info']['flypage'];
	}
	/**
...
*/
	...
	function image_tag($image, $args="", $resize=1, $path_appendix='product', $thumb_width=0, $thumb_height=0 ) {
		...
		return vmCommonHTML::imageTag( $url, '', '', '', '', '', '', $args );
...
//			}
	...
	function getPriceByShopperGroup( $product_id, $shopper_group_id, $check_multiple_prices=false, $additionalSQL='' ) {
		$use_alt_price = vmGet($_REQUEST, 'altprice', false);
		global $auth;
		...
		if( !isset($resultcache[$sig])) {
			...
			$price_info["product_price"]= $product_db_price * ((100 - $auth["shopper_group_discount"])/100);
			$price_info["product_currency"] = $db->f("product_currency");
			
			$price_info["product_base_price"]= $product_db_price * ((100 - $auth["shopper_group_discount"])/100);
	 ...
	 */
	function show_price( $product_id, $hide_tax = false, $template=0 ) {
        ...
        $tpl->set( 'price_table', $html);
    		...
    		if ($template == 0)
    		    $tmpl = 'common/price.tpl.php';
    		else
    		    $tmpl = 'common/price' . $template . '.tpl.php';    
        
		return $tpl->fetch($tmpl);
    ...
    function featuredProducts($random, $products, $categories) {
        ...
        if ( $category_id ) {
	        ...
	        $q  = "SELECT DISTINCT product_sku,#__{vm}_product.product_id AS product_id,product_name,product_s_desc, product_thumb_image, product_full_image, product_in_stock, product_url, category_id FROM #__{vm}_product ";
	        $q .= "LEFT JOIN #__{vm}_product_category_xref ON #__{vm}_product.product_id = #__{vm}_product_category_xref.product_id ";
	        $q .= "WHERE (#__{vm}_product.product_parent_id='' OR #__{vm}_product.product_parent_id='0') AND vendor_id='".$_SESSION['ps_vendor_id']."' ";
	        $q .= "AND #__{vm}_product.product_publish='Y' ";
	        $q .= "AND #__{vm}_product.product_special='Y' ";
	        if( CHECK_STOCK && PSHOP_SHOW_OUT_OF_STOCK_PRODUCTS != "1") {
		        $q .= " AND product_in_stock > 0 ";
	...
	function get_attributes( $product_id )
	{
	    ...
	    {
		if (!isset($_SESSION['products_info'][$child_id]['attributes']))
		{
		    $toCheck = true;
		    break;
		}
	    ...
	    while($db->next_record())
	    {
		$_SESSION['products_info'][$product_id]['attribute_list'][] = $db->f("attribute_name");
	    }
	    ...
	    $q = "SELECT product_id, attribute_name, attribute_value FROM #__{vm}_product_attribute WHERE ";
	    $q .= " product_id IN $childs_str";
	    
	    $db->query($q);
	    while($db->next_record())
	    {
		$_SESSION['products_info'][$db->f("product_id")]['attributes'][$db->f("attribute_name")] = $db->f("attribute_value");
	    }
	...
	function get_products_info_x( $product_ids )
	{
		...
		if (isset($_SESSION['products_info'][$product_id]['info']))
		    unset ($products_to_fetch[$product_id]);
	    }
	    ...
	    $q = "SELECT #__{vm}_product.*, #__{vm}_product_price.product_price, ";
	    $q .= "#__{vm}_product_discount.amount, #__{vm}_product_discount.is_percent ";
	    $q .= "FROM #__{vm}_product ";
	    $q .= "LEFT JOIN #__{vm}_product_price ON #__{vm}_product_price.product_id = #__{vm}_product.product_id ";
	    $q .= "LEFT JOIN #__{vm}_product_discount ON #__{vm}_product.product_discount_id = #__{vm}_product_discount.discount_id ";
	    $q .= "WHERE #__{vm}_product.product_id IN ";
	    $q .= $strProducts;
	    $q .= "OR #__{vm}_product.product_parent_id IN ";
	    $q .= $strProducts;
		...
		$_SESSION['products_info'][$db->f("product_id")]['info']['vendor_id'] = $db->f("vendor_id");
		$_SESSION['products_info'][$db->f("product_id")]['info']['product_parent_id'] = $db->f("product_parent_id");
		$_SESSION['products_info'][$db->f("product_id")]['info']['product_sku'] = $db->f("product_sku");
		$_SESSION['products_info'][$db->f("product_id")]['info']['product_s_desc'] = $db->f("product_s_desc");
		$_SESSION['products_info'][$db->f("product_id")]['info']['product_thumb_image'] = $db->f("product_thumb_image");
		$_SESSION['products_info'][$db->f("product_id")]['info']['product_full_image'] = $db->f("product_full_image");
		//$_SESSION['products_info'][$row->product_id]['info']['product_publish'] = $row->product_publish;
		$_SESSION['products_info'][$db->f("product_id")]['info']['product_weight'] = $db->f("product_weight");
		$_SESSION['products_info'][$db->f("product_id")]['info']['product_weight_uom'] = $db->f("product_weight_uom");
		$_SESSION['products_info'][$db->f("product_id")]['info']['product_length'] = $db->f("product_length");
		$_SESSION['products_info'][$db->f("product_id")]['info']['product_width'] = $db->f("product_width");
		$_SESSION['products_info'][$db->f("product_id")]['info']['product_height'] = $db->f("product_height");
		$_SESSION['products_info'][$db->f("product_id")]['info']['product_lwh_uom'] = $db->f("product_lwh_uom");
		$_SESSION['products_info'][$db->f("product_id")]['info']['product_url'] = $db->f("product_url");
		$_SESSION['products_info'][$db->f("product_id")]['info']['product_in_stock'] = $db->f("product_in_stock");
		$_SESSION['products_info'][$db->f("product_id")]['info']['product_available_date'] = $db->f("product_available_date");
		$_SESSION['products_info'][$db->f("product_id")]['info']['product_availability'] = $db->f("product_availability");
		$_SESSION['products_info'][$db->f("product_id")]['info']['product_special'] = $db->f("product_special");
		$_SESSION['products_info'][$db->f("product_id")]['info']['product_discount_id'] = $db->f("product_discount_id");
		//$_SESSION['products_info'][$db->f("product_id")]['info']['ship_code_id'] = $db->f("ship_code_id");
		$_SESSION['products_info'][$db->f("product_id")]['info']['cdate'] = $db->f("cdate");
		$_SESSION['products_info'][$db->f("product_id")]['info']['mdate'] = $db->f("mdate");
		$_SESSION['products_info'][$db->f("product_id")]['info']['product_name'] = $db->f("product_name");
		//$_SESSION['products_info'][$db->f("product_id")]['info']['product_sales'] = $db->f("product_sales");
		$_SESSION['products_info'][$db->f("product_id")]['info']['attribute'] = $db->f("attribute");
		$_SESSION['products_info'][$db->f("product_id")]['info']['custom_attribute'] = $db->f("custom_attribute");
		$_SESSION['products_info'][$db->f("product_id")]['info']['product_unit'] = $db->f("product_unit");
		$_SESSION['products_info'][$db->f("product_id")]['info']['child_options'] = $db->f("child_options");
		$_SESSION['products_info'][$db->f("product_id")]['info']['quantity_options'] = $db->f("quantity_options");
		$_SESSION['products_info'][$db->f("product_id")]['info']['child_option_ids'] = $db->f("child_option_ids");

		$_SESSION['products_info'][$db->f("product_id")]['info']['product_title'] = $db->f("product_title");
		$_SESSION['products_info'][$db->f("product_id")]['info']['product_metadesc'] = $db->f("product_metadesc");
		$_SESSION['products_info'][$db->f("product_id")]['info']['product_metakey'] = $db->f("product_metakey");
		$_SESSION['products_info'][$db->f("product_id")]['info']['product_abstract'] = $db->f("product_abstract");
		$_SESSION['products_info'][$db->f("product_id")]['info']['product_canonical'] = $db->f("product_canonical");
		$_SESSION['products_info'][$db->f("product_id")]['info']['product_price'] = $db->f("product_price");
		$_SESSION['products_info'][$db->f("product_id")]['info']['amount'] = $db->f("amount");
		$_SESSION['products_info'][$db->f("product_id")]['info']['is_percent'] = $db->f("is_percent");

		if ($db->f("product_parent_id") != 0)
		{
		    $_SESSION['products_info'][$db->f("product_parent_id")]['childs'][$db->f("product_id")] = $_SESSION['products_info'][$db->f("product_id")];
		}
