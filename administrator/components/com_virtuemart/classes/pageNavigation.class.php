...
function getLimitBox ( $link = '')
{
  ...
  if (vmGet( $_REQUEST, 'pshop_mode', null )=='admin') {
	...
	//$limits[JRoute::_($link.'&limit='.$i)] = $i;
      }
    }
  ...
  // build the html select list
    ...
    */
    $limits[/*vmRoute*/($link.'&limit=30')] = 30;
    $limits[/*vmRoute*/($link.'&limit=50')] = 50;
    ...
    foreach ($limits as $key => $value) {
      if($limit==$value) {$class='limitbox_active';} else {$class='limitbox';}
 ...
 return $html;
  ...
  function getPagesLinks($link='') {
    ...
    //echo $this->limitstart."  ".$this_page." ".$this->limit;
    ...
    if (($this_page > 1) && ($total_pages > $displayed_pages))  
    {
      $page = ($this_page - 2) * $this->limit;
      if( $link != '') {
        $html .= "<li><a href=\"".$link.'&limit='.$this->limit.'&limitstart='.$page."\" class=\"pagenav\" title=\"".$VM_LANG->_('PN_START')."\">&laquo;&laquo; ".$VM_LANG->_('PN_START')."</a></li>";
        
        //$html .= "\n<li><a href=\""./*JRoute::_*/($link.'&limit='.$this->limit.'&limitstart='.$page)."\" class=\"pagenav\" title=\"".$VM_LANG->_('PN_PREVIOUS')."\">&laquo; </a></li>";
        //$html .= "\n<li><a href=\"".($link.'/page' . $page . '-' . $this->limit)."\" class=\"pagenav\" title=\"".$VM_LANG->_('PN_PREVIOUS')."\">&laquo; </a></li>";
    ...
    for ($i=$start_loop; $i <= $stop_loop; $i++) {
        $page = ($i - 1) * $this->limit;
	$limits = '&limit='.$this->limit.'&limitstart='.$page;
	global $sess;
        $path = $sess->url($link.$limits);
      if ($i == $this_page) {
        $html .= "\n<li><span class=\"pagenav\"> $i </span></li>";
      } else {
        if( $link != '') {
          $html .= "\n<li><a href=\"".$path."\" class=\"pagenav\"><strong>$i</strong></a></li>";
        } else {
    ...
    if (($this_page < $total_pages) && ($total_pages > $displayed_pages)) 
    {
      $page = $this_page * $this->limit;
      $end_page = ($total_pages-1) * $this->limit;
      if( $link != '') {
        $html .= "\n<li><a href=\""./*JRoute::_*/($link.'&limit='.$this->limit.'&limitstart='.$page)."\" class=\"pagenav\" title=\"".$VM_LANG->_('PN_NEXT')."\"> &raquo;</a></li>";
