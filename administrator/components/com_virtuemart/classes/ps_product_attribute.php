...
class vm_ps_product_attribute {
	 ...
	 */
	function list_attribute( $product_id, $extra_ids = null,  $p_list = "") {
		...
		switch( $product_list) {
			...
			break ;
			case "YM" :
				return $this->list_attribute_list( $product_id, $display_use_parent, $product_list_child, $display_type, $class_suffix, $child_option_ids, $dw, $aw, $display_header, $product_list_type, $product_list, $p_list);
			break ;
			case "N" :
			default :
	 ...
	 * @return string HTML code with Items, attributes & price
	 */
	function list_attribute_drop( $product_id, $cls_suffix, $mode = "" ) {
		...
		$tpl->set( "advanced_attribute", $this->list_advanced_attribute( $product_id, $db->f( "product_id" ), $mode ) ) ;
		...
		if( $db->num_rows() > 0 ) {
			...
			$html .= "<label style=\"display:block;\" for=\"product_id_field\">" . $VM_LANG->_( 'PHPSHOP_PLEASE_SEL_ITEM' ) . "</label>" ;
			...
			while( $db->next_record() ) {
				...
				if( $selected ) {
					$tpl->set( "advanced_attribute", $this->list_advanced_attribute( $db->f( "product_id" ), null, $mode ) ) ;
	 ...
	 * @return string HTML code with Items, attributes & price
	 */
	function list_attribute_drop_multi( $product_id, $cls_suffix, $mode="" ) {
		...
		if( $db->num_rows() > 0 ) {
			...
			$html = "<input type=\"hidden\" name=\"product_id\" value=\"$product_id\" />" ;
			$html .= "<input type=\"hidden\" name=\"prod_id[]\" value=\"$product_id\" />" ;
			$html .= "<label  for=\"product_id_field\">" . $VM_LANG->_( 'PHPSHOP_PLEASE_SEL_ITEM' ) . "</label>" ;
	...
	function get_product_info_array($product_id)
	{
	    ...
	    $attrib = $ps_product->get_attributes($product_id);
	    $pinfo = $ps_product->get_product_info($product_id);
	    $ps_product_type->get_product_types($product_id);
	    
	    $Itemid = vmGet($_REQUEST, 'Itemid', "");
	    $price = $ps_product->get_product_price_x($product_id);
		...
		foreach($pinfo['childs'] as $child_id => $child_info)
		{
		    ...
		    $products[$ci]['parent_id'] = $child_info['info']['product_parent_id'];
		    ...
		    $productArray['product_id'] = $child_id;
		    
		    if (empty($child_info['info']['product_full_image']))
		    {
			$productArray['product_full_image'] = $_SESSION['products_info'][$child_info['info']['product_parent_id']]['info']['product_full_image'];
		    } else {
			$productArray['product_full_image'] = $child_info['info']['product_full_image'];
		    }
		    
		    if (empty($child_info['info']['product_thumb_image']))
		    {
			$productArray['product_thumb_image'] = $_SESSION['products_info'][$child_info['info']['product_parent_id']]['info']['product_thumb_image'];
		    } else {
			$productArray['product_thumb_image'] = $child_info['info']['product_thumb_image'];
		    }
		    $product_image = $tpl->vmBuildFullImageLink($productArray);
		    
		    $products[$ci]['product_image'] = $product_image;
		    
		    if (empty($child_info['info']['product_price']))
		    {
			$loc_price = $price['product_price'];
		    } else {
			$loc_price = $child_info['info']['product_price'];
		    }
		    $products[$ci]['actual_price'] = $CURRENCY_DISPLAY->getFullValue($loc_price);
		    $products[$ci]['price'] = $CURRENCY_DISPLAY->getFullValue($loc_price);
		    $products[$ci]['types'] = $ps_product_type->get_product_types($child_id);
	    ...
	    return array($ci, $products);
	}

	function list_attribute_list_x( $product_id, $display_use_parent, $child_link, $display_type, $cls_suffix, $child_ids, $dw, $aw, $display_header, $template)
	{
	    $ps_product = new ps_product();
	    $tpl = vmTemplate::getInstance();
	    $price = $ps_product->get_product_price_x($product_id);
	    ...
	    $product_arr = $this->get_product_info_array($product_id);
	    $child = $product_arr[0];
	    $products = $product_arr[1];
	    if ($child > 0)
	    {
		...
		$tpl->set( "child_count", $child == 0 ? 1 : $child );
	...
	function list_attribute_list_template( $product_id, $display_use_parent, $child_link, $display_type, $cls_sfuffix, $child_ids, $dw, $aw, $display_header, $template) {
		...
		if( ($db->num_rows() > 0) ) {
			...
			while( $db->next_record() ) {
				...
				$products[$ci]['advanced_attribute'] = $this->list_advanced_attribute( $db->f( "product_id" ), null, $mode ) ;
			...
			$html .= $this->list_advanced_attribute( $product_id, null, $mode);
	...
	function list_attribute_list( $product_id, $display_use_parent, $child_link, $display_type, $cls_suffix, $child_ids, $dw, $aw, $display_header, $product_list_type, $product_list, $mode="" ) {
		$template = "";
		if( $product_list == "Y" ) {
		    $template =  "product_details/includes/addtocart_list_single.tpl.php";
		} else {
		    $list_type = "multi" ;
		    $template = "product_details/includes/addtocart_list_multi.tpl.php";
		}
		
		$html =      $this->list_attribute_list_x($product_id, $display_use_parent, $child_link, $display_type, $cls_suffix, $child_ids, $dw, $aw, $display_header, $template);
	 ...
	 */
	function list_advanced_attribute( $product_id, $prod_id = null, $mode="" ) {
		global $CURRENCY_DISPLAY ;
		$ps_product = new ps_product( ) ;
		$db = new ps_DB( ) ;
		$auth = $_SESSION['auth'] ;
		...
		if (isset($GLOBALS['product_info'][$product_id]['attribute']))
		    $advanced_attribute_list = $GLOBALS['product_info'][$product_id]['attribute'];
		else {
		...
		if( $advanced_attribute_list ) {
			...
			if ($mode == "")
			    ...
			    return $tpl->fetch('browse/includes/addtocart_advanced_attribute.tpl.php');
		}
	}
	...
	function list_custom_attribute( $product_id, $prod_id = null ) {
		...
		if (isset($GLOBALS['product_info'][$product_id]['custom_attribute']))
		    $custom_attr_list = $GLOBALS['product_info'][$product_id]['custom_attribute'];
		else {
