...
class vm_ps_product_type {
	...
	*/
	...
	{
	    return count($this->get_product_types($product_id)) > 0;
	}
	
	function get_product_type_parameter($product_type_id)
	{
	    if (!isset($_SESSION['product_type_parameter'][$product_type_id]))
	    {
		$dbp = new ps_DB;
		$q = "SELECT * FROM #__{vm}_product_type_parameter ORDER BY product_type_id,parameter_list_order";
		$dbp->query( $q );
	    
		$last_id = -1;
		while ($dbp->next_record() )
		{
		    if ($dbp->f("product_type_id") != $last_id)
		    {
			$last_id = $dbp->f("product_type_id");
			$i = 0;
		    }
		    $_SESSION['product_type_parameter'][$dbp->f("product_type_id")][$i]['parameter_label'] = $dbp->f("parameter_label");
		    $_SESSION['product_type_parameter'][$dbp->f("product_type_id")][$i]['parameter_description'] = $dbp->f("parameter_description");
		    $_SESSION['product_type_parameter'][$dbp->f("product_type_id")][$i]['parameter_value'] = $dbp->f("parameter_value");
		    $_SESSION['product_type_parameter'][$dbp->f("product_type_id")][$i]['parameter_default'] = $dbp->f("parameter_default");
		    $_SESSION['product_type_parameter'][$dbp->f("product_type_id")][$i]['parameter_name'] = $dbp->f("parameter_name");

		    $i++;
		}
	    }
	    return $_SESSION['product_type_parameter'][$product_type_id];
	}
	...
	function get_product_types($product_id)
	{
	    ...
	    //unset($_SESSION['products_info'][$product_id]['product_types']);
	    if (!isset($_SESSION['products_info'][$product_id]['product_types']))
	    {
		$dbp =  new ps_DB;
		$q = "SELECT * from #__{vm}_product_product_type_xref LEFT join #__{vm}_product_type USING (product_type_id) ";
		$q.= "WHERE product_type_publish = 'Y' AND product_id='$product_id' ORDER BY product_id, product_type_list_order";
		$dbp->query ( $q );
	    
		while ($dbp->next_record() )
		{
		    $_SESSION['products_info'][$dbp->f("product_id")]['product_types'][$dbp->f("product_type_id")]['product_type_name'] = $dbp->f("product_type_name");
		    $_SESSION['products_info'][$dbp->f("product_id")]['product_types'][$dbp->f("product_type_id")]['product_type_description'] = $dbp->f("product_type_description");
		    $_SESSION['products_info'][$dbp->f("product_id")]['product_types'][$dbp->f("product_type_id")]['product_type_browsepage'] = $dbp->f("product_type_browsepage");
		    
		    $flypage_file = "";
		    if ($dbp->f("product_type_flypage")) 
			$flypage_file = VM_THEMEPATH."templates/".$dbp->f("product_type_flypage").".php";
		    $_SESSION['products_info'][$dbp->f("product_id")]['product_types'][$dbp->f("product_type_id")]['product_type_flypage'] = $flypage_file;
		}
		
		$prod_type = $_SESSION['products_info'][$product_id]['product_types'];
		if (count($prod_type) > 0)
		{
		    foreach($prod_type as $product_type_id => $prodt)
		    {
			$q2 = "SELECT * FROM #__{vm}_product_type_".$product_type_id;
			$q2 .= " WHERE product_id='$product_id'";
			$dbp->query($q2);
		    
			$pti = $this->get_product_type_parameter($product_type_id);
			
			$i = 0;
			foreach($pti as &$type_id)
			{
			    $_SESSION['products_info'][$product_id]['product_types'][$product_type_id]['parameters'][$i]['parameter_label'] = $type_id['parameter_label'];
			    $parameter_description = $type_id['parameter_description'];
			    $_SESSION['products_info'][$product_id]['product_types'][$product_type_id]['parameters'][$i]['parameter_description'] = $parameter_description;
			    if (!empty($parameter_description))
			    	$_SESSION['products_info'][$product_id]['product_types'][$product_type_id]['parameters'][$i]['tooltip'] = 
					vmToolTip($parameter_description, $VM_LANG->_('PHPSHOP_PRODUCT_TYPE_PARAMETER_FORM_DESCRIPTION'));
			    $_SESSION['products_info'][$product_id]['product_types'][$product_type_id]['parameters'][$i]['parameter_value'] = $dbp->f($type_id['parameter_name']);
			    $_SESSION['products_info'][$product_id]['product_types'][$product_type_id]['parameters'][$i]['parameter_unit'] = $type_id['parameter_unit'];
			    $i++;
			}
			$_SESSION['products_info'][$product_id]['product_types'][$product_type_id]['product_type_count_params'] = $i;
			
		    }
		}
	    }
	    return $_SESSION['products_info'][$product_id]['product_types'];
	}
	...
	function list_product_type($product_id) {
		...
		$prod_type = $this->get_product_types($product_id);

		if (count ($prod_type) == 0)
		    return;
	
		require_once(CLASSPATH . 'ps_product_attribute.php');
		$product_attribute = new ps_product_attribute();
		$ps_product = new ps_product();
		$childs_info = $product_attribute->get_product_info_array($product_id);
		
		$tpl->set( 'product_types', $prod_type );
		$tpl->set( 'childs_info', $childs_info );
