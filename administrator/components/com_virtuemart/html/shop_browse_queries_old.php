...
switch( $orderby ) {
	...
	case 'product_price':
		$orderbyField = '`#__{vm}_product_price`.$product_price_field'; break;
	case 'product_sku':
...
if ($vmxtsrch == 1) {
    ...
    $product_ids = $session->get('vm_product_ids', array(), 'unisearch');
    if (count($product_ids) > 0) {
	$where_clause[] = '`#__{vm}_product`.`product_id` IN ('. implode(', ', $product_ids). ')';
	$tmp_cat = $category_id;
	$category_id = '';
    }
}
...
if( $perm->is_registered_customer($auth['user_id']) ) {	
	$where_clause[] = "(`#__{vm}_product`.`product_id`=`#__{vm}_product_price`.`product_id` OR `#__{vm}_$product_price_field`.`product_id` IS NULL) ";
