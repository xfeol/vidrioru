...
class vmTheme extends vmTemplate  {
	...
	function vmlistAdditionalImages( $product_id, $images, $title='', $limit=1000 ) {
		...
		foreach( $images as $image ) { 
			...
			//$thumbtag = ps_product::image_tag( $image->file_url, 'class="browseProductImage"', 1, 'product', $image->file_image_thumb_width, $image->file_image_thumb_height );
			$thumbtag = ps_product::image_tag( $image->file_url, 'class="browseProductImage" height="100" width="100"', 1, 'product', 100, 100 );
