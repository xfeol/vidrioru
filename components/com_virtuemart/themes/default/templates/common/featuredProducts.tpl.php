...
foreach( $featured_products as $featured ) {
	...
	<div style="float:left;width:<?php echo $cellwidth ?>%;text-align:top;padding:0px;" >
         <a title="<?php echo $featured["product_name"] ?>" href="<?php $sess->purl(URL."index.php?option=com_virtuemart&amp;page=shop.product_details&amp;flypage=".$featured["flypage"]."&amp;product_id=".$featured["product_id"]."&amp;category_id=".$featured["category_id"]) ?>"> 
			<h4><?php echo $featured["product_name"] ?></h4></a>
			<?php echo $featured['product_price'] ?><br />
            <?php
			if ( $featured["product_thumb"] ) { ?>
                <a title="<?php echo $featured["product_name"] ?>" href="<?php $sess->purl(URL."index.php?option=com_virtuemart&amp;page=shop.product_details&amp;flypage=".$featured["flypage"]."&amp;product_id=".$featured["product_id"]."&amp;category_id=".$featured["category_id"]) ?>"> 
				<?php echo ps_product::image_tag( $featured["product_thumb"], "class=\"browseProductImage\" alt=\"".$featured["product_name"]."\"");
				?></a><br /><br/>
            <?php
			}?>
