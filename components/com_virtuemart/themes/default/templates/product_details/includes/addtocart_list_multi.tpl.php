...
// Start Ouputing the Child Details
?>
    ...
    <!--div class="vmCartChild<?php echo $cls_suffix." ".$product['bgcolor'].$cls_suffix ?>"-->
        <form action="<?php echo $mm_action_url ?>index.php" method="post" name="addtocart" id="addtocart<?php echo $product['product_id'] ?>" class="addtocart_form" <?php if( $this->get_cfg( 'useAjaxCartActions', 1 ) && !$notify ) { echo 'onsubmit="handleAddToCart( this.id );return false;"'; } ?>>
            <div class="vmCartChildElementM">
                ...
                <div style="float:left;text-align:left;margin:0 auto;">
                ...
                (!$product['advanced_attribute'])) { 
                	...
                	<input type="submit" class="addtocart_button_small" value="" title="<?php echo $button_lbl ?>" />
                    </div>
                <?php }
                ...
                if( $_SESSION['auth']['show_prices'] && _SHOW_PRICES) {  ?>           
                    ...
                    if( $product['price'] != $product['actual_price'] ) { ?>
                        ...
                        &nbsp;&nbsp;
                    <?php } 
                    echo "<strong>" . $product['actual_price'] . "</strong>" ?></div>    
                ...
                if (USE_AS_CATALOGUE != '1' ) { ?>
                    ...
                    <!--span class="vmChildDetail<?php echo $cls_suffix ?>" style="visibility:hidden; float: right;text-align: right;margin-top: 0px;"><?php echo $product['quantity_box'] ?></span-->
                <?php } ?>
            <!--/div-->
            <br style="clear: both;">
