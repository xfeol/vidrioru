...
class modAiDaNews2Helper{
	...
	function imgpreflist_findlink ($p, $txt, $cimg, $uid, $num) {
		...
		while ($i < 5 && empty($imgurl)) {
			if ($p[$i] == 1) { //Default
				...
				if ($cimg) $imgurl = 'images/stories/' . $cimg;
	...
	function getList(&$params) {
		...
		if (is_array($secid)) $secid = implode(", ", $secid);
		$aid		= $user->get('aid', 0);
		...
		{
			case 'by_me':
				$wauth = ' AND (a.created_by = ' . (int) $userId . ' OR a.modified_by = ' . (int) $userId . ')';
				if (!$userId) $wauth = ' AND (a.created_by = 0)';
				break;
			case 'not_me':
				$wauth = ' AND (created_by <> ' . (int) $userId . ' AND modified_by <> ' . (int) $userId . ')';
				break;
			case 'sel':
				if (!$params->get('sauth')) {
					...
					$wauth = " AND a.created_by IN ( $authors )";
				}
				break;
		}
		...
		if ($params->get('foffset')) {
			...
			$relnorepeat .
			$relatedcond;
			if ($params->get('show_trash') == 1) $query .= ' AND a.state = 1';
			if ($params->get('show_trash') == 2) $query .= ' AND a.state <> 1';
		...
		// Content Items only
		$query = 'SELECT DISTINCT a.*, r.rating_count, r.rating_sum, cc.image AS catimg, cc.title AS cattle, s.title AS sectit, ' . $oc . 
			' CASE WHEN CHAR_LENGTH(a.alias) THEN CONCAT_WS(":", a.id, a.alias) ELSE a.id END as slug,'.
			' CASE WHEN CHAR_LENGTH(cc.alias) THEN CONCAT_WS(":", cc.id, cc.alias) ELSE cc.id END as catslug'.
			' FROM #__content AS a';
			if($params->get('show_front')) {
				if($params->get('show_front') == 1) $query .= ' LEFT JOIN #__content_frontpage AS f ON f.content_id = a.id';
				if($params->get('show_front') == 2) $query .= ' INNER JOIN #__content_frontpage AS f ON f.content_id = a.id';
			...
			($catid ? $catCondition : '').
			($secid ? $secCondition : '').
			($params->get('show_front') == 1 ? ' AND f.content_id IS NULL ' : '').
			' AND s.published = 1' .
			' AND cc.published = 1' .
			( $recent ? " AND DATEDIFF(".$db->Quote($now).", a.created) < " . $recent : '' );
			if ($params->get('foffset') && $feats) $query .= ' AND a.id NOT IN (' . $feats . ')';
			if ($params->get('show_trash') == 1) $query .= ' AND a.state = 1';
			if ($params->get('show_trash') == 2) $query .= ' AND a.state <> 1';
			$query .= $relnorepeat .
			$relatedcond .
		...
		/* LINKS CREATION */
			...
			if ($params->get('myblogurl')) {
				...
				$artlink = myGetPermalinkUrl($row->id);
			}else{
				if($row->access <= $aid) {
					$artlink = JRoute::_(ContentHelperRoute::getArticleRoute($row->slug, $row->catslug, $row->sectionid));
				} else {
					$artlink = JRoute::_('index.php?option=com_user&view=login');
				}
			}
			...
			/* SECTION LINK */
				...
				$seclink = JRoute::_('index.php?option=com_content&view=section&id=' . $row->sectionid);
			...
			/* KUNENA LINK */
				...
				$kunlink = JRoute::_('index.php?option=com_kunena&func=profile&userid=' . $row->created_by);
		...
		if ($chimg1) {
			...
			if($img1url) {
				...
				if ($params->get('img1lnk') == 1) {
					...
					$img1url = '<a class="aidanews2_img1" href="' . $seclink . '">' . $img1url . '</a>';
		...
		if ($chimg2) {
				...
				$img2url = "";
				$img2url = modAiDaNews2Helper::imgpreflist_findlink($pref2, $row->introtext . ' ' . $row->fulltext, $row->catimg, $row->created_by, 2);
			...
			if($img2url) {
				...
				}elseif ($params->get('img2lnk') == 3) {
					$img2url = '<a class="aidanews2_img2" href="' . $seclink . '">' . $img2url . '</a>';
		...
		if ($chimg3) {
				...
				$img3url = "";
				$img3url = modAiDaNews2Helper::imgpreflist_findlink($pref3, $row->introtext . ' ' . $row->fulltext, $row->catimg, $row->created_by, 3);
			...
			if($img3url) {
				...
				}elseif ($params->get('img3lnk') == 3) {
					$img3url = '<a class="aidanews2_img3" href="' . $seclink . '">' . $img3url . '</a>';
			...
			if ($params->get('linksec'))
				$lists[$i]->section = '<span class="aidanews2_section aidasec_' . $row->sectionid . '"><a href="' . $seclink . '">' . $row->sectit . '</a></span>';
			else
				$lists[$i]->section = '<span class="aidanews2_section aidasec_' . $row->sectionid . '">' . $row->sectit . '</span>';
			...
			/* AUTHOR */
				...
				if ($alias) {
					...
					if ($params->get('authlnk') == 1)
						$auth = '<a href="' . $cblink . '">' . $auth . '</a>';
					elseif ($params->get('authlnk') == 2)
						$auth = '<a href="' . $jslink . '">' . $auth . '</a>';
					elseif ($params->get('authlnk') == 3)
						$auth = '<a href="' . $jsslink . '">' . $auth . '</a>';
				}
				
			$lists[$i]->author = '<span class="aidanews2_author">' . $auth . '</span>';
