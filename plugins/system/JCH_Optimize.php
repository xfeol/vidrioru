...
require_once ( dirname(__FILE__).DS.'JCH_Optimize'.DS.'CSS.php' );
require_once ( dirname(__FILE__).DS.'JCH_Optimize'.DS.'HTML.php' );
require_once ( dirname(__FILE__).DS.'JCH_Optimize'.DS.'jsmin.php' );
...
class  plgSystemJCH_Optimize extends JPlugin {
    ...
    protected function processLink($sType, $sCacheGroup, $sLink, $sLnEnd, $iCnt='') {
        ...
        if($bCached) {
            $sUrl	=	$this->buildUrl($sFileName, $sType, $this->isGZ());
     ...
     */
    protected function buildUrl($sFile, $sType, $bGz=false) {
        $sPath	=	JURI::base(true).'/plugins/system/JCH_Optimize/';
        if($this->params->get('htaccess', 0)) {
            $sUrl   = $sPath.($bGz ? 'gz/' : 'nz/').$sFile.'.'.$sType;
     ...
     * @param string $sGroup  Name of cache group
     * @return string        Cache file name
     */
    protected function getFilename($sId) {
        $oStorage    =   $this->oCache->_getStorage();
        $oConfig    =& JFactory::getConfig();
	$sHash      = $oConfig->getValue('config.secret');
        $sName  =   md5($oStorage->_application.'-'.$sId.'-'.$sHash.'-'.$oStorage->_language);
    ...
    public function isGZ() {
        ...
        if($iIsGz) {
            $bEncoding	=	JResponse::_clientEncoding();
            if (!$bEncoding) {
                return false;
            }
        ...
        foreach($aUrlArray as $sUrl) {
            ...
            if(file_exists($aMatches[0])) {
                ...
                if($sType == 'css') {
                    ...
                    $sContent    =   preg_replace_callback('~url\s?\([\'"]?(?![a-z]+:|/+)([^\'")]+)[\'"]?\)~i',
                            array($this, 'correctUrl'), $sContent);
                    if(@$this->aMedia[$sUrl]){
    ...
    protected function generateSprite($sCss, $sLnEnd) {
        ...
        require_once ( dirname(__FILE__).DS.'JCH_Optimize'.DS.'css-sprite-gen.inc.php' );
        ...
        for ($i = 0; $i < count($aSpriteCss); $i++) {
            if(@$aSpriteCss[$i]) {
                ...
                preg_match('~(?<=background)(?:[^;]+?)(
                            ...
                            |rgb\([^\)]+?\))
                            (?:[^;]*?);~ix',
                        $aDeclaration[$i], $aBgColor);
                preg_match('~(?<=background)(?:[^;]+?)\s(
                                scroll|fixed
                                )\s(?:(?:[^;]+?)?;)~ix',
                        $aDeclaration[$i], $aBgAttach);
